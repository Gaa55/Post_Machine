...
class MainActivity : AppCompatActivity() {
    ...
    private var operationCarrige: OperationCarriage = OperationCarriage(-1)
    ...
    private fun applyCommands() {
        ...
        if (commands.getCommands()[lastCommandComplete].getOperationId() == 2) {
            ...
            ((layoutContainer.getChildAt(carriage.getCarriageLocation()) as RelativeLayout).getChildAt(
                1
            ) as RelativeLayout).getChildAt(0).visibility =
                View.VISIBLE
        }
        ...
        for (i in 0..lineSize) {
            ...
            ((newRelativeLayout.getChildAt(1) as RelativeLayout)).setOnClickListener {
                ...
                if (line[i].getChecked()) {
                    (newRelativeLayout.getChildAt(1) as RelativeLayout).getChildAt(0).visibility =
                        View.VISIBLE
                } else {
                    (newRelativeLayout.getChildAt(1) as RelativeLayout).getChildAt(0).visibility =
                        View.INVISIBLE
    ...
    fun commandsIniit() {
        ...
        for (i in -1..commands.getCommands().indices.last) {
            if (i == -1) {
                val newRelativeLayout =
                    layoutInflater.inflate(R.layout.command_card_header, null) as LinearLayout
                val params = ActionBar.LayoutParams(
                    ActionBar.LayoutParams.WRAP_CONTENT,
                    ActionBar.LayoutParams.WRAP_CONTENT
                )
            ...
            val newRelativeLayout =
                layoutInflater.inflate(R.layout.command_card, null) as LinearLayout
            val params = ActionBar.LayoutParams(
                ActionBar.LayoutParams.WRAP_CONTENT,
                ActionBar.LayoutParams.WRAP_CONTENT
            )
            params.setMargins(0, 0, 0, 0)
            newRelativeLayout.layoutParams = params
            (((newRelativeLayout.getChildAt(0) as LinearLayout).getChildAt(1) as TextView)).text =
                i.toString();
            (((newRelativeLayout.getChildAt(0) as LinearLayout).getChildAt(3) as TextView)).text =
                commands.getCommands().get(i).getOperationIcon()
            if (commands.getCommands().get(i).getOperationId() != 4) {
                (((newRelativeLayout.getChildAt(0) as LinearLayout).getChildAt(5) as TextView)).text =
                    commands.getCommands().get(i).getLink().toString()
            } else {
                (((newRelativeLayout.getChildAt(0) as LinearLayout).getChildAt(5) as TextView)).text =
                    commands.getCommands().get(i).getLink()
                        .toString() + " " + commands.getCommands().get(i).getLink2().toString()
            }
            layoutContainer.addView(newRelativeLayout)
        }
    }
    ...
    private fun refreshLine() {
        ...
        for (i in line.indices) {
            line[i].setUncheck()
            ((layoutContainer.getChildAt(i) as RelativeLayout).getChildAt(1) as RelativeLayout).getChildAt(
                0
            ).visibility =
                View.INVISIBLE
        }
    ...
    private fun carriageUpdate() {
        ...
        ((layoutContainer.getChildAt(carriage.getCarriageLocation()) as RelativeLayout).getChildAt(2) as RelativeLayout).visibility =
            View.VISIBLE

        if (carriage.getLastCarriageLocation().size != 0)
            for (i in carriage.getLastCarriageLocation())
                ((layoutContainer.getChildAt(i) as RelativeLayout).getChildAt(2) as RelativeLayout).visibility =
                    View.INVISIBLE
        ...
        findViewById<Button>(R.id.createBtn).setOnClickListener() {
            ...
            if (findViewById<EditText>(R.id.editTextNumber2).visibility == View.INVISIBLE) {
                if (findViewById<EditText>(R.id.editTextNumber).text.toString() != "") {
                    commands.addCommand(
                        findViewById<Spinner>(R.id.spinner).selectedItemPosition,
                        findViewById<EditText>(R.id.editTextNumber).text.toString().toInt()
                    )
                    commandsIniit()

                }
            }
            if (findViewById<EditText>(R.id.editTextNumber).text.toString() != "" && findViewById<EditText>(
    ...
    private fun operationCarriageUpdate() {
        ...
        if (operationCarrige.getCarriageLocation() != -1)
            ((layoutContainer.getChildAt(operationCarrige.getCarriageLocation() + 1) as LinearLayout).getChildAt(
                0
            ) as LinearLayout).getChildAt(0).visibility =
                View.VISIBLE

        if (operationCarrige.getLastCarriageLocation().size != 0)
            for (i in operationCarrige.getLastCarriageLocation())
                ((layoutContainer.getChildAt(i + 1) as LinearLayout).getChildAt(0) as LinearLayout).getChildAt(
                    0
                ).visibility =
                    View.INVISIBLE
